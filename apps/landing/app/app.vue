...
const title = appConfig.site?.name || 'TurntableHQ'
const description = appConfig.site?.description || 'Build amazing products faster with our powerful platform.'
    ...
    <UFooter>
            ...
            <div>
              ...
              <ul class="space-y-2">
                <li v-for="link in footerLinks.product" :key="link.label">
                  <ULink :to="link.to" class="text-sm text-muted hover:text-foreground">
                    {{ link.label }}
                  </ULink>
                </li>
              </ul>
      ...
      <template #left>
        <p class="text-sm text-muted">
          © {{ new Date().getFullYear() }} TurntableHQ. All rights reserved.
        </p>
      </template>
